...
class CurrentWeather:
    """
        ...
        and API Key is of trial version per day it will give 50 limits. It has API endpoints with jwt Token
        """
    ...
    def __init__(self, city_name, email):
    ...
    @staticmethod
    def get_weather_report(api_response, place_name, email):
        """
        ...
        weather = Weather(email, place_name, weather_text, weather_icon, observation_date_time, temperature, real_feel_temperature,
                          real_feel_temperature_shade, relative_humidity, indoor_relative_humidity, dew_point, wind, wind_gust, uv_index,
    ...
    def access_api(self):
        ...
        get_weather_report =  CurrentWeather.get_weather_report(current_condition_api_response, self.city_name, self.email)
...
def current_weather():
    if request.method == "GET" or request.method == "POST":
        ...
        if not data or not data.get('email') or not data.get('city_name'):
        ...
        logger.debug(f"Email: {data.get('email')} city_name: {city_name}")
        weather = CurrentWeather(city_name, data.get('email'))
        logger.debug(f"weather: {weather}")
        weather_report = weather.access_api()
        logger.debug(f"weather_report: {weather_report}")
